...
class Rabbit:public Live
{
	...
	const static int r_lifespan = 12;
	const static int rep_adulty = 3;
	const static int hungry = 10;
...
public:
	...
	void run_away(std::pair<Live*, Live*> mat_g[], std::list<Live*>* animals) ;
	int get_fullness(){ return fullness; };
	virtual bool get_gen();
